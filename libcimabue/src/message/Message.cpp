...
Message::Message(message_t msg_type, string fromClient, string toClient,
                 string fromProxy, string toProxy, string fromServer, string toServer,
...
void Message::copy(Message &to)
{
	...
	to.setProxySource(proxySrc);
	to.setProxyDest(proxyDest);
...
// Proxy Source
string Message::getProxySource()
{
	return proxySrc;
}

void Message::setProxySource(string s)
{
	proxySrc = s;
}

// Proxy Dest
string Message::getProxyDest()
{
	return proxyDest;
}

void Message::setProxyDest(string d)
{
	proxyDest = d;
}
...
string Message::serialize()
{
	...
	serialized += MSG_SEPARATOR + proxySrc;
	serialized += MSG_SEPARATOR + proxyDest;
...
void Message::deserialize(string s)
{
	...
	tokens.pop_front();
	proxySrc = tokens.front(); // proxySrc
	tokens.pop_front();
	proxyDest = tokens.front(); // proxyDest
	tokens.pop_front();
...
message_t Message::encodeType(std::string s)
{
	...
	else if (s == "MSG_ADD_PROXY")
		return MSG_ADD_PROXY;

	else if (s == "MSG_REM_PROXY")
		return MSG_REM_PROXY;
	...
	else if (s == "MSG_PING_PROXY")
		return MSG_PING_PROXY;
...
void Message::dump()
{
	...
	messageLog.print(LOG_DEBUG, "proxySrc:\t%s\n", proxySrc.c_str());
	messageLog.print(LOG_DEBUG, "proxyDest:\t%s\n", proxyDest.c_str());
